...
export function createGitHubClient({
  ...
  async function request(path, { search = false, code = false, raw = false } = {}) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const wait = Math.max(0, nextRequest - now(), search ? nextSearch - now() : 0);
      ...
      if ([301, 302, 307, 308].includes(response.status)) {
        ...
        const error = new Error("GitHub API redirect rejected: invalid destination or redirect limit exceeded");
        ...
        if (redirects >= 3 || !destination ||
            destination.origin !== "https://api.github.com" ||
            destination.username || destination.password) throw error;
      ...
      const message = token ? String(detail).replaceAll(token, "[redacted]") : String(detail);
      const error = new Error(`GitHub ${response.status}: ${message.slice(0, 220)}`);
      error.status = response.status;
      ...
      if (rateLimited) {
        ...
        const primaryWait = remaining === "0" && Number.isFinite(reset)
          ? Math.max(0, reset - now()) : 0;
        ...
        const backoff = Math.max(retry || 60000 * 2 ** attempt, primaryWait) + 500;
        error.retryAfterMs = backoff;
        if (attempt === maxAttempts - 1 || rateLimitWaited + backoff > maxRateLimitWaitMs) {
          rateLimitBlocked = error;
          throw error;
        }
        rateLimitWaited += backoff;
        onRetry({ status: response.status, waitMs: backoff, attempt: attempt + 1 });
        await sleep(backoff);
        continue;
      }
