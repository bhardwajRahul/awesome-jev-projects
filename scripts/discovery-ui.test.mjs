...
test('daily discovery is identical in four locales while existing Header, Sponsor and search remain intact', async () => {
  ...
  try {
    ...
    for (const initialLocale of ['zh', 'en', 'ja', 'ko']) {
      ...
      assert.ok(html.includes('class="hero-curator-seal"') && html.includes('@0xLogicrw'), `${initialLocale}: Curator seal is present`);
      ...
      assert.match(html, /<button[^>]*class="jevy"/);
      assert.doesNotMatch(html, /<div[^>]*class="jevy"/);
