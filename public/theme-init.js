...
function apply(){const theme=resolved();root.dataset.theme=theme;root.dataset.themeMode=mode;for(const button of document.querySelectorAll("[data-theme-toggle]")){const label=button.getAttribute(theme==="dark"?"data-theme-light-label":"data-theme-dark-label");if(label){button.setAttribute("aria-label",label);button.setAttribute("title",label)}}window.dispatchEvent(new CustomEvent("awesome-jev-theme-change",{detail:{mode,resolved:theme}}))}
