...
[![Curated Projects](https://img.shields.io/badge/收录项目-169%2B-blue?style=flat-square)](#目录)
...
**精选由 TypeSafe AI Jev 大模型驱动的 169+ 个开源工具、生产级应用与实验项目雷达。**
...
- [🛠️ SDK 与决策框架 (11)](#sdk-decision-frameworks-zh)
- [🔌 SDK 与生态框架集成 (8)](#sdk-integrations-zh)
- [💻 命令行与自动化工作流 (26)](#cli-pipelines-zh)
- [💾 数据库扩展与语义搜索 (4)](#data-search-zh)
- [🌐 浏览器与桌面端侧自动化 (14)](#browser-os-action-zh)
- [🧹 上下文垃圾回收与降噪 (7)](#context-gc-filter-zh)
- [🛡️ 安全审查与护栏防御 (9)](#security-guardrails-zh)
- [🧩 MCP 协议与工具扩展 (28)](#mcp-integrations-zh)
- [🧭 代码库分析与图谱寻路 (9)](#codebase-graph-pathfinding-zh)
- [🔀 模型路由与成本优化 (9)](#routing-cost-optimization-zh)
- [📊 垂直行业与金融工具 (15)](#domain-vertical-tools-zh)
- [🎯 通用决策辅助与评估 (10)](#decision-tools-zh)
- [🏷️ 文本分类与分类学标注 (2)](#classification-taxonomy-zh)
- [📈 评测基准与可观测性 (1)](#evaluation-observability-zh)
- [🎙️ 语音交互与会话流 (1)](#voice-conversation-zh)
- [🎨 创意生成与多媒体 (2)](#creative-tools-zh)
- [📖 本地运行与架构](#本地运行与架构)
- [🤝 如何提交你的项目](#如何提交你的项目)
...
- [**jev-doom-agent**](https://github.com/lukaske/jev-doom-agent) `★ 1` - 在浏览器里跑两份 Doom 引擎，让 Jev 根据游戏状态选择战术动作。
  - 🎯 **Jev 决策点**: 读取结构化血量、弹药和可见目标，从战术宏中选一项，再交给本地控制器执行。
  - 💡 **核心收益**: 能在相同起点对照不同策略，并查看每次模型决策。

- [**jev-gomoku**](https://github.com/XieChengYuan/jev-gomoku) `★ 1` - 弈瞬：双 Jev 玩家在九宫格五子棋中对战，比较不同输入信息对落子决策的影响，并逐手展示真实请求与返回。
  - 🎯 **Jev 决策点**: 每盘每手回答一个 Choice 问题：根据棋盘、规则、颜色和合法候选选择落子点，由本地代码校验执行。
  - 💡 **核心收益**: 逐手展示真实请求与返回概率，支持免 API Key 真实对局回放与 OpenRouter 自定义对战，支持 JSON 导出。
  - 🌐 [在线体验 Demo / 演示看板](https://xiechengyuan.github.io/jev-gomoku/)

- [**jev-little-airways**](https://github.com/lbotinelly/jev-little-airways) `★ 1` - 小岛机场模拟器：让 Jev 判断飞机是否改降、盘旋、让行，以及谁先落地。
  - 🎯 **Jev 决策点**: 每架飞机提交油量、预计时间和附近交通，塔台另问降落顺序。
  - 💡 **核心收益**: 把多飞机冲突做成可观察的决策场景，每次请求都能展开查看。
...
- [**jev-demos**](https://github.com/Bud-ro/jev-demos) - 用迷宫考 Jev：只问下一步，或一次问后面很多步，看看它何时撞墙、绕路或反复打转。
  - 🎯 **Jev 决策点**: 把迷宫状态与多步候选动作交给 Jev，再由模拟器逐步执行和判分。
  - 💡 **核心收益**: 用明确的合法移动规则比较不同问法，展示空间推理的局限。

- [**jev-experiments**](https://github.com/mittal-parth/jev-experiments) - 让 Jev 玩 Chrome 小恐龙和本地射击竞技场：读取结构化状态，选跳跃、移动、瞄准和开火。
  - 🎯 **Jev 决策点**: 每个 tick 并行询问动作与相关开关，Python 或页面代码执行物理和碰撞规则。
  - 💡 **核心收益**: 把持续游戏决策做成带日志和观察面板的可试验循环。
...
- [**openai-scala-client**](https://github.com/cequence-io/openai-scala-client) `★ 248` - 让 Scala 应用也能接入 Jev。这个多模型客户端新增了独立 TypeSafe 模块，用状态和判断题获取结构化答案。
  - 🎯 **Jev 决策点**: 调用 System One 返回单选、评分和是非概率；可把受支持的封闭 JSON Schema 转成判断题。
  - 💡 **核心收益**: 能沿用 Scala 的异步接口和错误处理；Jev 是额外支持的 provider，不承担普通聊天生成。

- [**pi-fabric**](https://github.com/monotykamary/pi-fabric) `★ 233` - 给 Pi 的工具运行时加上可编程的 Jev 决策循环。先写好观察、判断和执行步骤，再让它按预算在前台或后台运行。
  - 🎯 **Jev 决策点**: 对程序提交的状态做单选、是非判断或评分；循环和动作执行由本地程序控制。
  - 💡 **核心收益**: 适合把反复发生的小判断写成可复用流程；Jev 是需要配置的可选能力。
...
- [**effect-agent**](https://github.com/danieljvdm/effect-agent) `★ 116` - 在 Effect Agent 里接入 Jev 决策模型。TypeScript 程序可以用带类型的问题集做判断，也能把它用于可选的模型选择。
  - 🎯 **Jev 决策点**: TypeSafeDecisionModel 通过 TypeSafeClient 调用 Jev，并把概率、单选与评分结果接回 Effect。
  - 💡 **核心收益**: 复用 Effect 的配置、类型和错误处理；重试与超时由应用自己设置。

- [**jev-visual**](https://github.com/hr98w/jev-visual) `★ 95` - 在 Apple Silicon Mac 本地跑的视觉版 Jev 实验。对单张图片做选择、打分与是非判断，自带 3 个本地视觉游戏 Demo。
  - 🎯 **Jev 决策点**: 复用图像上下文，读取模型 logits 直接为候选答案打分，由本地代码拼装结构化结果。
  - 💡 **核心收益**: 把 Jev 的多问题单次打分范式拓展到端侧视觉场景，本地秒级跑通分拣与体感游戏。

- [**advocaat**](https://github.com/pithings/advocaat) `★ 66` - 用简短的 TypeScript 调用向 Jev 提问。把同一份数据里的多个判断一次写好，直接拿到概率、选项和分数。
  - 🎯 **Jev 决策点**: 将带类型的问题转成 System One 请求，并把命名答案映射回调用方。
  - 💡 **核心收益**: 减少手写请求和解析代码，也支持通过 Vercel 网关调用。

- [**jev-java**](https://github.com/Olti1947/jev-java) `★ 2` - jev-java：由业务代码定义问题，客户端负责提交 Jev 请求并解析结构化结果。
  - 🎯 **Jev 决策点**: 由业务代码定义问题，客户端负责提交 Jev 请求并解析结构化结果。
  - 💡 **核心收益**: 在现有程序中复用接入代码，减少重复处理接口细节。

- [**jev-starter**](https://github.com/hamakyo/jev-starter) `★ 1` - 给 Jev 判断补上应用里的后半段：把结果送去自动处理、备用模型或人工复核，并记录每条规则的效果。
  - 🎯 **Jev 决策点**: 为分类或检查任务提供选项与概率；代码里的阈值决定后续处理路线。
  - 💡 **核心收益**: 把业务规则留在代码里，便于评估误判、转人工比例和不同阈值的取舍。

- [**jevclient**](https://github.com/AboveColin/jevclient) `★ 1` - 在 Python 异步程序里调用 Jev 的小客户端。一次发出多个判断问题，直接拿到分类、分数和概率对象。
  - 🎯 **Jev 决策点**: 围绕同一份状态回答是非、单选和评分问题，把结果交还 Python 程序继续处理。
  - 💡 **核心收益**: 省去自己封装请求和解析回答的工作；可复用 aiohttp 连接，重试策略由调用方掌握。
...
- [**jev-playground**](https://github.com/mizchi/jev-playground) `★ 14` - 一个 MoonBit 实验场：让 Jev 下五子棋、打简化 MOBA、判断命令风险，也试着把判断写进小语言。
  - 🎯 **Jev 决策点**: 把棋盘、游戏状态、命令或候选任务转成类型化问题，让 Jev 选动作或给风险评分。
  - 💡 **核心收益**: 把不同问题的问法、结果和限制放到可重跑的小实验里比较。
...
- [**SemDecide**](https://github.com/sharziki/semdecide) `★ 5` - 给终端管道加一点语义判断：一句话分类、一批文本过滤，都能接在 Bash 和 CI 后面。
  - 🎯 **Jev 决策点**: 对输入做分类或概率判断，再由本地规则执行阈值、退出码与 allow/escalate/block。
  - 💡 **核心收益**: 不用搭一个完整 Agent，也能把语义判断嵌进现有脚本。

- [**jev-lm**](https://github.com/y0usaf/jev-lm) `★ 4` - 把下一个词当选择题，试着用 Jev 拼出句子；也能让它挑选本地草拟的整段续写。
  - 🎯 **Jev 决策点**: Choice 选词，Noul 判断候选续写能否接上以及是否该停止。
  - 💡 **核心收益**: 用一套小实验看清决策模型拿来生成文字时会遇到什么问题。
...
- [**jev-chat**](https://github.com/adhyaay-karnwal/jev-chat) `★ 3` - 一个研究聊天解码器：不断让 Jev 选词或短语，再由代码接成回答。
  - 🎯 **Jev 决策点**: 从分层候选表选下一段内容，或一次选定完整回复。
  - 💡 **核心收益**: 把选答案和生成语言的差别变成可查看的实验记录。

- [**jev-cli**](https://github.com/tumf/jev-cli) `★ 2` - 在终端发一段文本或 JSON，直接拿回「是不是、选哪个、打几分」。输出能接进脚本继续处理。
  - 🎯 **Jev 决策点**: 将单题或多题状态发送给 Jev，返回 Noul、Choice、Score 的完整 JSON 或主值。
  - 💡 **核心收益**: 只用 Python 标准库提供输入、退出码和结构化错误，方便管道调用。
...
- [**jev-pref**](https://github.com/doeixd/jev-pref) `★ 2` - jev-pref：把 AGENTS.md 中的项目偏好转为 jev-pref.json 规则，用 Jev 审查代码变更（hunk、暂存文件、PR），并把发现反馈给编码 Agent。
  - 🎯 **Jev 决策点**: 对每条偏好规则和每个代码变更片段，判断该变更是否违反这条规则。
  - 💡 **核心收益**: 以 CLI 形式运行（npx jev-pref setup），可接入 pre-commit、PR 与 Agent 工作流，区分阻断与建议级发现。

- [**jev-system-one**](https://github.com/haseeb-heaven/jev-system-one) `★ 2` - 在终端里提问：OpenAI 写回答，Jev 负责定回答方式、检查草稿并决定是否重写。
  - 🎯 **Jev 决策点**: 先判断模式、深度与澄清需求，再检查草稿是否切题、有无无依据陈述。
  - 💡 **核心收益**: 回答和决策报告并排显示，方便看模型为什么进入下一步。
...
- [**pi-fast-jev-compaction**](https://github.com/joelhooks/pi-fast-jev-compaction) `★ 2` - 给 Pi 删掉过时的工具调用和结果，保留原话，不另外生成摘要。
  - 🎯 **Jev 决策点**: 对每个工具调用分别问是否保留调用与结果，再执行保留、截短结果或成对删除。
  - 💡 **核心收益**: 减少送给主模型的旧工具内容，并保留可重建的裁剪记录。
...
- [**ask-jev**](https://github.com/omni-/ask-jev) `★ 1` - 在 Windows Codex 会话里输入 :jev，让它按已记录的执行证据做一次检查。
  - 🎯 **Jev 决策点**: 向 Jev 批量询问测试、执行与证据是否支持结论，也可单独问一个是非题。
  - 💡 **核心收益**: 把“Agent 说做了”与日志里实际留下的证据分开检查。

- [**is-odd-jev**](https://github.com/alxcrt/is-odd-jev) `★ 1` - 一个刻意小题大做的实验：把数字交给 Jev，问它是不是奇数，再看返回的概率。
  - 🎯 **Jev 决策点**: 用 Noul 回答 Is n odd，本地按概率转成布尔值。
  - 💡 **核心收益**: 用极小接口示例观察模型判断与确定性计算的差别。
...
- [**jev-cli**](https://github.com/jtsang4/jev-cli) `★ 1` - 在终端里问 Jev 判断题。输入文本或 JSON，再给出分类、是非或评分问题，拿回脚本能直接读取的 JSON。
  - 🎯 **Jev 决策点**: 针对同一份输入做分类、真假判断和分级评分，返回选项及其概率。
  - 💡 **核心收益**: 能接收标准输入，把语义校验接进已有脚本和 CI；支持直连 TypeSafe 或走 Vercel 网关。
...
- [**jev-code**](https://github.com/rhighs/jev-code) `★ 1` - 一个实验性编程终端：让 Jev 做受限选择，逐步拼出 AST，并调用本地工具。
  - 🎯 **Jev 决策点**: 在有限语法和工具候选里选下一步，代码负责组合成 Python、Bash 等输出。
  - 💡 **核心收益**: 能观察结构化决策如何参与代码生成和执行。
...
- [**TypeSafe AI Playground**](https://github.com/markjaquith/typesafe-ai-playground) `★ 1` - 一个 Rust 实验箱：查隐私信息、看代码注释、判断表达是否友善，直接在终端试 Jev。
  - 🎯 **Jev 决策点**: 把短任务拆成概率或类别判断，例如是否含 PHI、注释是否有用、语气是否友善。
  - 💡 **核心收益**: 用可运行的小实验理解结构化判断能放在哪里。

- [**jevscript**](https://github.com/amberwhitehead/jevscript) - 一个把语义判断写进程序语言的早期实验。目前先用手写脚本验证：几个问题合在一起问 Jev 会怎样。
  - 🎯 **Jev 决策点**: M0 脚本对同一工单发送 Choice、Noul、Score，比较合并请求与逐题请求。
  - 💡 **核心收益**: 给后续解释器设计建立接口与批量请求的基线。
...
*原生数据库 SQL 语义扩展、数据表行级智能过滤与重排检索。*

- [**pg-jev**](https://github.com/realZachi/pg-jev) `★ 137` - 让 SQL 直接问「哪些工单的客户正在生气」。用一句人话给数据库行筛选、分类和排序。
  - 🎯 **Jev 决策点**: 把行内容成批交给 Jev，返回是否符合条件、所属类别或等级分数，供 WHERE、GROUP BY、ORDER BY 使用。
  - 💡 **核心收益**: 语义条件可以和原有 SQL 一起写，并复用同一会话里已经算过的判断。

- [**jev-search**](https://github.com/superagents-lab/jev-search) `★ 38` - 用人话搜网页，先挑搜索源和时间范围，再把相关链接排到前面。页面给链接和摘要，不代写答案。
  - 🎯 **Jev 决策点**: Jev 对请求做结构化判断以选择检索参数，再给 Search1API 返回的标题和摘要评分。
  - 💡 **核心收益**: 把来源选择与结果排序串起来，同时保留可改的过滤条件和失败来源提示。
...
- [**jevql**](https://github.com/kylemclaren/jevql) `★ 2` - 针对原生 Postgres 的 Jev 语义 SQL 扩展。无需装插件，在终端和 Go/TS/Python SDK 中直接用 WHERE jev() 做语义过滤、排序与分组。
  - 🎯 **Jev 决策点**: 对 SQL 初筛后的每一行数据调用 Jev 进行 Noul、Choice 或 Score 判断，结果用于客户端的语义过滤、概率排序或分组。
  - 💡 **核心收益**: 纯客户端/双阶段执行，支持高并发批量请求与本地缓存（重复内容不重复打分），附带开箱即用的在线 Demo。
  - 🌐 [在线体验 Demo / 演示看板](https://jevql.fly.dev/)

- [**llama-index-jev**](https://github.com/WiktorB2004/llama-index-jev) `★ 2` - llama-index-jev：给候选记录判断相关性或打分，再由本地程序筛选和排序。
  - 🎯 **Jev 决策点**: 给候选记录判断相关性或打分，再由本地程序筛选和排序。
  - 💡 **核心收益**: 把语义判断接进已有的数据查询流程。
...
- [**grokskill-jev**](https://github.com/AE-AlphaEdge/grokskill-jev) - 把 Jev Ultrafast 接到 Grok Build，复用已打开的 Chrome 标签页，并提供 PowerShell 启动方式。
  - 🎯 **Jev 决策点**: 沿用上游：Jev 选择点击、选择或完成动作，需要写文字时再调用小模型。
  - 💡 **核心收益**: 为 Grok 和 Windows 使用流程补上 skill、启动器及浏览器复用。

- [**jev-macos-loop**](https://github.com/jcpsimmons/jev-macos-loop) - 在 Mac 本地识别屏幕文字和控件，把文字选项交给 Jev，再点击它选中的元素。
  - 🎯 **Jev 决策点**: Jev 从本地视觉、OCR 与无障碍标签组成的有限元素列表中选择；坐标和输入执行留在 Mac。
  - 💡 **核心收益**: 让原生界面点击使用可枚举目标，并在执行前复核焦点与元素状态。
...
- [**fast-jev-compaction**](https://github.com/tamaratran/fast-jev-compaction) `★ 2645` - 给 Claude Code 压缩上下文时，先删掉过时的工具调用和结果。留下来的原文不改写，关键对话仍按原样保留。
  - 🎯 **Jev 决策点**: 分别判断一条工具调用是否还要记住、它的结果是否仍需完整保留；本地规则决定保留、截短或删除。
  - 💡 **核心收益**: 用原文筛选替代部分摘要，减少路径、报错和命令在改写中走样。

- [**skillbox**](https://github.com/kitze/skillbox) `★ 149` - 自建一个有版本管理的 Agent 技能库，还能选配 Jev 推荐：告诉它当前任务，从你有权限使用的技能里挑更相关的。
  - 🎯 **Jev 决策点**: 对候选技能逐项评估任务相关度，再由应用整理推荐结果。
  - 💡 **核心收益**: 技能查找不只依赖关键词；未配置或调用失败时仍可使用确定性搜索。

- [**bluenoise**](https://github.com/rokcso/bluenoise) `★ 82` - 推特/X 浏览器的降噪过滤扩展。本地规则过滤垃圾，疑难回复成批交给 Jev 判定噪音概率（>=0.9 则隐藏）。
  - 🎯 **Jev 决策点**: 对未命中本地规则的长尾回复，单次最多批量提交 25 条询问 noul 噪音概率，由代码决定是否隐藏。
  - 💡 **核心收益**: 不调用任何 X 官方 API，用本地混合架构实现毫秒级社交媒体时间线清洁。

- [**Winnow**](https://github.com/GhalebDweikat/winnow) `★ 13` - 给 Claude Code 的长日志装一道筛子。暂时不相关的内容先藏起来，想看时还能完整找回。
  - 🎯 **Jev 决策点**: 逐块判断 Read、Bash、Grep 输出是否有用；保留相关或不确定内容，隐藏高置信度无关块。
  - 💡 **核心收益**: 减少进入上下文的冗余输出，并保留可召回的原文。

- [**jevlogs**](https://github.com/reachjalil/jevlogs) `★ 5` - 日志照样归档，先让 Jev 挑出值得继续分析的部分，再决定哪些送给更重的模型。
  - 🎯 **Jev 决策点**: 评估诊断价值、优先级与可行动概率，本地规则注释日志或路由到分析分支。
  - 💡 **核心收益**: 在保留原始日志链路的同时，把后续分析注意力集中到高价值事件。
...
- [**jev-skill-gate**](https://github.com/ShivamPansuriya/jev-skill-gate) `★ 2` - 技能装得多，先别把说明全塞给 Claude Code。它按当前项目给技能排相关度，保留有用的说明，其余只留名字或手动入口。
  - 🎯 **Jev 决策点**: 结合项目技术栈、目录和 README，逐个判断已安装技能是否相关，再由规则调整可见程度。
  - 💡 **核心收益**: 减少进入上下文的技能描述，同时保留手动调用入口；没有 API key 时可用本地评分。

- [**jev-context**](https://github.com/zbush/jev-context) `★ 1` - 给 Codex 的代码搜索加一道筛子。先用 ripgrep 找候选，再只把 Jev 判为相关的片段送回来。
  - 🎯 **Jev 决策点**: 逐段判断代码是否与问题相关，只返回 Yes 片段，并记录筛选前后的文本。
  - 💡 **核心收益**: 能按指定 tokenizer 核对取回文本的变化；不等同整项任务的成本节省。
...
*基于 Model Context Protocol (MCP) 的标准服务器与工具插件。*
...
- [**vellum-assistant**](https://github.com/vellum-ai/vellum-assistant) `★ 1285` - Vellum Assistant 增加了一个 Jev provider。选用它后，可以把对话状态或明确的问题包交给 Jev，获得结构化判断。
  - 🎯 **Jev 决策点**: 调用原生 System One 接口回答命名问题，并通过适配层把结果送回现有助手接口。
  - 💡 **核心收益**: 能在同一个助手框架里试用 Jev；这证明兼容接入，不代表助手默认由 Jev 驱动。

- [**ai**](https://github.com/hackclub/ai) `★ 133` - Hack Club 的 AI 代理增加了 Jev 转发接口。获准用户可通过统一入口提交判断题，同时走现有的鉴权、限额和用量记录。
  - 🎯 **Jev 决策点**: 代理把状态与问题转发给 TypeSafe，再将 Jev 的结构化答案返回调用方。
  - 💡 **核心收益**: 接入已有账户和配额体系；固定版本里的 Jev 通道处于受开关控制的封闭测试。

- [**taskuary**](https://github.com/ldbumble/taskuary) `★ 102` - Taskuary 把消息、任务和 Agent 工作汇到一起，并提供可选 Jev 判断器，对一次运行的状态检查是否满足用户设定的条件。
  - 🎯 **Jev 决策点**: 把同一份运行状态与多条条件提交为是非问题，返回布尔结果及原始概率。
  - 💡 **核心收益**: 让任务路由使用可查看的判断信号；Jev 不承担助手的聊天或写作。

- [**jev-mcp**](https://github.com/jkudish/jev-mcp) `★ 67` - 给 Agent 配三个小助手：核对引用、筛查注入指令、从候选里挑出相关结果。
  - 🎯 **Jev 决策点**: 通过 verify、screen、find 分别给证据支持度、内容风险和候选相关性做判断。
  - 💡 **核心收益**: 让每次读取和引用都能加一道可配置的检查。
...
- [**typesafe-mcp**](https://github.com/itsmostafa/typesafe-mcp) `★ 59` - 给 Claude Code、Claude Desktop 和 Codex 接上一个选择题外脑：选哪个、打几分、是不是，都用同一个 MCP 工具问。
  - 🎯 **Jev 决策点**: 把状态和 typed questions 交给 Jev，返回 Choice、Score、Noul 及概率。
  - 💡 **核心收益**: 拿到可以直接写进分支判断的答案，省去解析一大段文字。
...
- [**synkora-ai**](https://github.com/getsynkora/synkora-ai) `★ 34` - Synkora Agent 平台提供 TypeSafe 客户端，可把业务数据交给 Jev 做分类、评分或是非判断，作为工作流中的一项能力。
  - 🎯 **Jev 决策点**: 把调用方的 question、options 和 levels 转成 Jev 的题型与标准，再调用 System One。
  - 💡 **核心收益**: 应用可以沿用统一的凭据和结果格式；这是兼容接入，不代表整个平台默认使用 Jev。

- [**Jevbridge**](https://github.com/gamesonrblx/Jevbridge) `★ 13` - 给现有 Agent 接一个结构化决策适配器：同样的问题可以交给 Jev，也可以交给普通模型或离线规则。
  - 🎯 **Jev 决策点**: 对状态回答 Choice、Score、Noul，再按置信度把动作建议分成执行、确认、升级或放弃。
  - 💡 **核心收益**: 统一决策接口，便于试验任务路由、工具门禁和界面动作选择。

- [**jev**](https://github.com/dannote/jev) `★ 10` - 把 Jev 变成 Elixir 程序里的一个异步同伴。发去状态和问题，答案回来后直接用模式匹配决定下一步。
  - 🎯 **Jev 决策点**: 把 Noul、Choice、Score 的回复转成 Elixir map，再由 GenServer 的 handle_answer 与 guard 路由。
  - 💡 **核心收益**: 沿用 OTP 的消息、监督和错误处理方式接入模型判断。

- [**jev-mcp**](https://github.com/blakestone-x/jev-mcp) `★ 7` - 给 MCP 客户端加上分类、打分、是非判断和候选匹配工具。
  - 🎯 **Jev 决策点**: 将工具参数转成 Jev 的 Choice、Score、Noul 问题，返回结构化答案和概率。
  - 💡 **核心收益**: Agent 拿到能直接用于分支判断的结果，不用再解析一段回答。

- [**pi-jev**](https://github.com/TheoOliveira/pi-jev) `★ 6` - 帮 Pi 编程 Agent 按任务找工具和技能。还提供结构化判断，并可选择开启模型分流与工具历史筛选。
  - 🎯 **Jev 决策点**: 判断哪些未启用工具、技能和历史记录与当前任务有关，供本地规则决定加载或保留。
  - 💡 **核心收益**: 让能力按需进入工作流；自动模式需主动开启，Jev 不可用时有本地或原生流程回退。

- [**zod-jev**](https://github.com/jomatsu/zod-jev) `★ 6` - 给 Zod 表单校验补上「意思对不对」这一层：字段格式过关后，再看描述是否匹配、内容是否含个人信息。
  - 🎯 **Jev 决策点**: 把同一次解析的语义规则合并成 Jev Noul 问题，再按概率映射成通过、拒绝、不确定或不可用。
  - 💡 **核心收益**: 继续使用熟悉的 Zod 错误对象，把语义问题定位到具体字段。
...
- [**jev-mcp**](https://github.com/rashedInt32/jev-mcp) `★ 2` - 把 Jev 装成通用 MCP 判断工具。Agent 可以拿它做分类、打分、是非检查，也能一次提交多个问题。
  - 🎯 **Jev 决策点**: 在调用方提供的选项和评分标准内作答，返回答案、概率分布及可供规则使用的判断信号。
  - 💡 **核心收益**: 现有 MCP 客户端就能接入；带选项校验和弃权结果，方便处理不确定的回答。

- [**jev-workbench**](https://github.com/molis-ai/jev-workbench) `★ 2` - 在本地网页里定义、试跑并发布 Jev 判断函数，后端和 Agent 调用同一个固定版本。
  - 🎯 **Jev 决策点**: 把工单分类、证据判断等问题保存为 Noul、Choice 或 Score 函数，再调用 TypeSafe。
  - 💡 **核心收益**: 同一套判断不用在每个 Agent 或后端里重写，调用方版本也能固定。

- [**laravel-typesafe-jev**](https://github.com/Butochnikov/laravel-typesafe-jev) `★ 2` - 把 Jev 接入 Laravel，提供配置、依赖注入、Facade 和可记录请求的测试替身。
  - 🎯 **Jev 决策点**: 复用社区 PHP SDK 发出三类判断请求，保留类型、异步 Promise 和异常。
  - 💡 **核心收益**: Laravel 服务与队列任务可以沿用自己的配置和测试方式。
...
- [**jev_ampcode**](https://github.com/thesammykins/jev_ampcode) `★ 1` - 给 Amp 一个比较方案的小工具。把备选方案、证据和偏好摆出来，让 Jev 帮忙选。
  - 🎯 **Jev 决策点**: 在明确候选中分配选择概率，并分别检查各方案是否符合要求。
  - 💡 **核心收益**: 把方案取舍记录成可对照的判断；结果只作建议，不授权执行。
...
- [**jev-go**](https://github.com/Stumble/jev-go) `★ 1` - 社区 Go SDK 和交互命令行，既能直连 TypeSafe，也能走 Vercel AI Gateway。
  - 🎯 **Jev 决策点**: 用 Go 类型构建 Choice、Noul、Score 请求，解析并校验返回值。
  - 💡 **核心收益**: Go 项目可以直接接入 Jev，并处理超时、重试和请求取消。

- [**jev-go**](https://github.com/guillemus/jev-go) `★ 1` - 一个接口很小的社区 Go SDK，用来发 Jev 请求和读取模型列表。
  - 🎯 **Jev 决策点**: 将 Go 的 Choice、Noul、Score 问题转成 System One API 请求。
  - 💡 **核心收益**: 给 Go 项目提供最基本的 Jev 调用入口。
...
- [**jev-resilience**](https://github.com/Vicente-MD/jev-resilience) `★ 1` - 给 Spring WebFlux 检查“HTTP 200 但正文其实报错”的响应。
  - 🎯 **Jev 决策点**: Jev 判断响应正文是否是隐藏错误或维护通知，超过阈值就抛出业务异常。
  - 💡 **核心收益**: 让现有错误处理能看到状态码掩盖的失败。

- [**jevgo**](https://github.com/fgn/jevgo) `★ 1` - 一个只依赖 Go 标准库的 Jev 客户端，可选接上 Langfuse 记录调用。
  - 🎯 **Jev 决策点**: 发送结构化状态和三类问题，返回带类型的答案，并检查响应是否符合请求。
  - 💡 **核心收益**: 把连接、校验、取消和观测接入 Go 服务。
...
- [**WrongStack**](https://github.com/WrongStack/WrongStack) `★ 327` - 给 WrongStack 编程 Agent 增加一个可选分派助手。遇到多个相近的专长 Agent 时，用 Jev 判断谁更适合当前任务。
  - 🎯 **Jev 决策点**: 从候选角色中选一个，同时判断有没有任何候选真正适合；不合适时交回原有策略。
  - 💡 **核心收益**: 分派结果带有可检查的概率，也能明确表示没有合适人选。

- [**Jev Codex Router**](https://github.com/0xNatoshi/jev-codex-router) `★ 26` - 每轮先看任务有多难，再给 Codex 选模型和思考深度。简单活省一点，难活再上强模型。
  - 🎯 **Jev 决策点**: Jev 判断当前轮次的任务层级与推理需求，本地策略决定模型、effort 和速度档。
  - 💡 **核心收益**: 作者对 237 轮历史会话按标价重算，估计比全用最贵模型省约 60%。

- [**jev-demo**](https://github.com/minghanminghan/jev-demo) - 一个客服分流小样：同一轮先问全套分类问题，再沿答案选路径；用户想转人工时就提前交接。
  - 🎯 **Jev 决策点**: 并行评估分类树各层，同时判断转人工意愿和挫败程度，再由本地规则选用需要的答案。
  - 💡 **核心收益**: 把多层分流问题合并询问，避免每走一层就重新请求。

- [**jev-router-playground**](https://github.com/hugo-alves/jev-router-playground) - 把几种模型放进候选池，让 Jev 选一个，再把各模型的实际回答摆出来由你评判。
  - 🎯 **Jev 决策点**: 对任务与候选模型描述做适配度判断，本地整理概率并选择候选；可继续调用候选模型作比较。
  - 💡 **核心收益**: 留下路由选择与人工偏好的对照记录，便于检查选得是否合适。
...
- [**hermes-jev-north-star**](https://github.com/poponline63/hermes-jev-north-star) `★ 1` - hermes-jev-north-star：评估业务状态，给出供本地规则参考的分类或风险分数。
  - 🎯 **Jev 决策点**: 评估业务状态，给出供本地规则参考的分类或风险分数。
  - 💡 **核心收益**: 增加一组可记录、可对照的判断信号。

- [**jev-broadcast-lab**](https://github.com/4anti/jev-broadcast-lab) `★ 1` - 一个以国际象棋为主的 Jev 实验台，也能试工单分类、文档匹配和审核。
  - 🎯 **Jev 决策点**: 从 chess.js 算出的合法走法中选择一步；Stockfish 分数只给操作者看。
  - 💡 **核心收益**: 可以同时观察模型选步和本地棋力评估，检查两者的差别。
...
- [**jev-agent-failure-benchmark**](https://github.com/TokenTrim/jev-agent-failure-benchmark) `★ 1` - 给失败的多 Agent 记录找原因：哪个 Agent、哪一步、哪种错误。
  - 🎯 **Jev 决策点**: 每条轨迹问三道 Choice，分别选择责任 Agent、关键步骤和错误类型。
  - 💡 **核心收益**: 把故障归因做成可重复评分的实验，保留失败请求与结果。

- [**jev-playground**](https://github.com/Little-Planet-Labs/jev-playground) `★ 1` - 在网页里贴一段状态、添加选择题或评分题，直接看 Jev 的答案和概率分布。
  - 🎯 **Jev 决策点**: 把多道 Noul、Choice、Score 问题放进同一次请求。
  - 💡 **核心收益**: 不用先写业务代码就能调整问题、候选和评分标准。
...
- [**jev-predict-skill**](https://github.com/DanielKillenberger/jev-predict-skill) `★ 1` - 一个 Agent skill：读另一个 skill 的规则和现有证据，预测它下一次会选哪个结论。
  - 🎯 **Jev 决策点**: 先判断任务能否变成有限选项，再从目标 skill 原有结论里选一项。
  - 💡 **核心收益**: 在不执行目标 skill 的情况下，先得到一次可检查的候选判断。

- [**jev-rerank-bench**](https://github.com/anessbelbati/jev-rerank-bench) `★ 1` - 把搜索出来的三十段文字交给 Jev，测试它能否像专用 reranker 一样排出相关内容。
  - 🎯 **Jev 决策点**: 用相关性评分、是非判断或 Choice 对相同候选重排，并记录原始响应。
  - 💡 **核心收益**: 能按数据集查看质量、延迟、成本和置信区间。

- [**jev-research**](https://github.com/sherajdev/jev-research) `★ 1` - 一份 Jev 与 Herdr 协作指南，附带把任务分给不同 Agent 的小原型。
  - 🎯 **Jev 决策点**: 根据任务和仓库状态选择执行者、判断风险和是否准备好派发。
  - 💡 **核心收益**: 给多 Agent 调度提供一个能看懂、能改的起点。
